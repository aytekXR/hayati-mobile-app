...
/**
 * ⚠️ **EVERY LANE BELOW IS A DISCLOSURE QUESTION, AND THIS IS WHERE IT IS ASKED**
 * (ADR-068 Decision 3).
 *
 ...
 * privacy notice"* is a legal judgement **per field**, over the 75 fields in this
 * file (counted two ways, both agreeing; ADR-068 D3 carries the command), and a registry would move that judgement rather than remove it. ADR-067's
 ...
 *
 * **If you add a lane below: say whether the notice names it, and file an issue
 * if it does not.** That is the whole ask.
 *
 * ⚠️ It sits above the FIRST export interface, not a convenient one. The
 * built-diff review found the first draft placed below `ExportConsent`,
 * `ExportDevice` and `ExportPushDiagnostic` — which are precisely the three
 * lanes behind #249 and #226. A note about disclosure gaps, sitting under the
 * interfaces that caused them, is decoration.
 */
/**
