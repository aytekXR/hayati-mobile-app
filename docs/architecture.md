...
**Firebase environments (M1.1):** two Firebase projects map to the two Dart-entrypoint flavors; options live as committed per-flavor Dart files (`app/lib/core/firebase/firebase_options_{dev,prod}.dart`, FlutterFire Dart-only init — no `GoogleService-Info.plist` needed to build; config files are not secrets, API keys get console-side restrictions). Environment bindings (e.g. the Firebase-backed `AuthRepository`) are composed in the entrypoints via `runHayati(..., extraOverrides: [...])`; base providers throw when unoverridden (the `appConfigProvider` pattern). *Deviation from plan:* founder `firebase login` was unavailable in Session 003, so per the resume-prompt fallback the options files hold placeholders (dev projectId `demo-hayati` runs the Auth emulator credential-free) until real provisioning in M1.2 (issue #5); the auth flow's runtime verification path is the emulator integration test (`app/integration_test/`, CI wiring = ci-debt #6). Firebase iOS SDK 12 raised the iOS floor: deployment target pinned to **15.0** in the Runner pbxproj (iOS plugins integrate via **Swift Package Manager** — the Flutter 3.44 scaffold is SwiftPM-first and has no Podfile; adding one breaks the build, verified on CI in Session 003).
