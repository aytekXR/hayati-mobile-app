...
- **F4 Paywall & subscription.** RevenueCat. Free: daily question + streak. Premium (one purchase covers both partners): all packs, quizzes, AI coach, spice mode, timeline. Trial 7 days, annual-first presentation. **Premium is couple-scoped: one purchase entitles both partners — that IS the "gift your partner" flow** (ADR-015, Session 017). There is no store gifting mechanism for auto-renewable subscriptions (no StoreKit gifting API, no App Store Connect giftable toggle); App Store Review Guideline 3.1.1 permits buyer≠beneficiary by name, provided the money moves through IAP — which it does. The non-purchasing partner unlocks off the couple's entitlement mirror without ever seeing a paywall (regression-pinned in the app suite). Family Sharing stays OFF on the products (irreversible once enabled).
- **F5 AI Coach v0.** Chat with persona presets: Coach (communication help), Date Genie (locale-aware date ideas), Gift Genie (occasion-aware). Server-proxied LLM; premium-only with a 30 msgs/day cap (free tier: zero — scope per `mvp.md` IN #7); hard guardrails: not therapy disclaimer, crisis-language detector → localized redirect to professional help, no medical/legal advice, respects tone register.
