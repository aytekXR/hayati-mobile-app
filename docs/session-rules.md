...
2. Read `resume-prompt.md`. It contains exactly one objective. **That objective is the session. Nothing else is.**
   - **`session-context.md`** — the standing operating context: toolchain and commands, the machine, review discipline, the binding invariants, and the never-without-asking list. Read it before touching anything; it is what `resume-prompt.md`'s header used to carry.
   - **`session-lessons.md`** — the numbered institutional lessons. Skim the "recurring shape" section at minimum; the resume prompt cites individual lessons by number. **Append to it at close, never renumber it.**
   *(Both were split out of `resume-prompt.md` on 2026-08-05: the header had grown to six wall-of-text standing notes and 80 addenda, and the objective was hard to find inside it.)*
...
2. Regenerate `resume-prompt.md`: inspect `roadmap.md` + current milestone progress + open `ci-debt` issues → choose the single highest-priority next task → write it with concrete acceptance criteria (rule #3). One objective only. **Standing content goes in `session-context.md` and lessons go in `session-lessons.md` — do not let either creep back into the prompt's header.** If a lesson was learned, append it to `session-lessons.md` with the next number and cite it from the prompt by number.
...
5. `gh run watch` until the pipeline concludes. Green → session over. Red → quick fix (≤15 min) now and re-push; structural → `gh issue create --label ci-debt` with failing-log excerpt, note it in `past-prompts.md`, and only then end (rule #5). A session never ends with an *unexamined* red pipeline. **Watch the post-merge MAIN run, not only the PR run** — `integration-emulator` is main-only by cost design (ADR-006), so the PR's green says nothing about it. Capture the run id right after the merge (`gh run list --workflow ci.yml --branch main --limit 1 --json databaseId -q '.[0].databaseId'`) and watch *that*. Since ADR-024 D8 the close commit no longer *cancels* it — but it still concludes after the session's attention has moved on, which is exactly why the Slack notifier exists.
