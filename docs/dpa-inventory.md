...
| **Google — Firebase Cloud Messaging** | Processor | The device's FCM **registration token** (the address of one physical phone) and the **content-free** notification payload — `composePush` has no question/answer parameter, so no relationship content can enter one (ADR-012 F6, structural) | **NOT region-pinned** | Same Google Cloud DPA | Same. **Added 2026-08-21 (S082, ADR-058).** It had no row: the register disposed of the whole delivery leg in a subordinate clause of a note, which is a claim about *coverage* and not a disclosure of a *recipient* |
| **Apple — Apple Push Notification service (APNs)** | ⚠️ **Role TO BE CONFIRMED — founder/lawyer (lawyer question E)** | The APNs device token and the content-free notification payload, relayed to the subscriber's iPhone | Apple's own infrastructure, **not EU-pinned** | Apple's developer terms. **Whether this leg is a processor relationship or Apple-as-its-own-controller is NOT settled here** — the register already treats Apple as an independent controller for *App Store* data, and APNs is a different leg under different terms. Asserting either would be playing counsel | Founder/lawyer: settle the role (question E in `docs/legal/README.md`); if processor, confirm whether a cross-border safeguard leg is needed for it distinctly from the Google legs. **Added 2026-08-21 (S082, ADR-058)** |
...
| **Mixpanel / product analytics (when built)** | Processor (planned) | Pseudonymous product events; no answer/reflection/coach text, ever (`architecture.md` §7 binding) | To be determined when built | **Still a placeholder — no processor exists, and that is now a narrower statement than it was.** Corrected 2026-08-18 (S081, ADR-057): the app **is** instrumented — eight §7 events with typed payloads and real call sites — but there is still **no analytics SDK and no vendor sink anywhere**, prod is wired to a **no-op**, and the dev sink writes one `debugPrint` line and reaches nothing else (asserted: no file under `core/analytics/` so much as names `CrashReporter`). **Nothing leaves the device, so no processor is engaged and no row is due yet.** The trigger is precise: the first vendor adapter | **Before any adapter merges** (not after): it arrives with its own separate, unbundled opt-in consent (not the special-category consent); accept the analytics DPA; add a cross-border leg if applicable; and add the `docs/legal/` collection line — which bumps `CURRENT_LEGAL_VERSION` and re-gates every existing user, so it is a founder/lawyer decision, exactly like #226. **ADR-057 records that this gate is a paragraph, not a CI check** — now also tracked as issue **#247**, because three prose statements and no issue is how a dependency gets forgotten. **The S082 draft (ADR-058) does NOT discharge this row:** it corrects what the notice says about the *instrumentation* (recorded on the device, discarded, no provider), and deliberately names **no vendor** — a technology intention in `architecture.md` is not an engaged recipient. Whether the adapter then needs its own bump is **lawyer question D** |
...
- **Device-registration state, corrected 2026-08-17 (S071).** This note previously read *"`fcmTokens` / push delivery is not in the register: the device-side token capture does not exist, nothing writes the field"* — and that stopped being true at **ADR-042/ADR-044**, when the `registerPushToken` callable and the app-side capture shipped and were wired in both flavor entrypoints. Nothing is *stored* today only because no device has ever produced a token (4 accounts, 0 registered, measured 2026-08-17), which is a device-side gap rather than a missing writer. **`fcmTokens` and `pushDiagnostic` are therefore listed in the Firestore row above**, as data categories that the system writes the moment a device registers.

- **Delivery now has its own two rows, 2026-08-21 (S082, ADR-058).** This note used to end *"Notification delivery remains a Google/Apple leg already covered by the DPAs above"* — a statement about **coverage**, standing in for a disclosure of a **recipient**. The privacy policy has to tell the subject *"who it is shared with, at home and abroad"*, and it cannot be built on a register in which the two parties that carry every notification are named nowhere. **Google — Firebase Cloud Messaging** and **Apple — APNs** are now rows. Apple's role on that leg is entered as *to be confirmed*, not guessed.
- **Sharpened while adding them:** build 119 — the only binary on any phone — already calls `requestPermission()` and `registerPushToken`, so the collection this register describes is one the **shipped app already attempts**, not one that waits on a future build. `pushDiagnostic` is the half that is not yet on a phone (ADR-049 merged after the build was cut).
- **İYS / ETK — a recorded position, not advice.** The four `PushKind` members (`partnerAnswered`, `reveal`, `dailyQuestion`, `streakAtRisk`) are **service messages about the recipient's own activity in a product they signed up for**, not *ticari elektronik ileti*. On that reading İYS registration is not triggered by anything built today. **Any promotional push would trigger it**, and that stays a forward obligation. The founder's lawyer settles it; the engineering fact is that no promotional push kind exists and the vocabulary is closed.
- **What this register does NOT settle** (issue **#226**): the privacy policy's *"what we collect"* list names neither field. This is an engineering register; changing the **notice** bumps `CURRENT_LEGAL_VERSION` and re-gates consent for every existing user (ADR-023's three-way source sentinel), which is a founder/lawyer decision and not a side effect a diagnostics slice may cause. **Still open — but a reviewable correction now exists.** S082 drafted the version-3 revision at `docs/legal/proposed/` (ADR-058): it names both fields, names the two delivery recipients above, states the bounds, and **deliberately does not bump the version**. It is a draft awaiting the founder and the lawyer, not a landed change.
