...
| **ADR-055** | `integration-emulator` bounds each suite with `tool/ci/integration_watchdog.sh` so a wedge **FAILS** (named) instead of being **CANCELLED** (silent) — `slack_notify.sh` sends nothing for `cancelled`, correctly, and GitHub uses that word for a timeout too. ⚠️ The per-suite bounds must keep summing to **less** than `timeout-minutes`, or the watchdog can never fire and the guard is dead; `integration_watchdog_test.sh` asserts it. Do **not** raise the ceiling instead. |
