...
1. Flutter scaffold, flavors (dev/prod), folder layout per `architecture.md` §2; lint config (incl. RTL `start/end` lint); Riverpod + codegen wiring.
...
**Accept:** rules tests prove non-members can't read couple docs and preview never leaks answer content; integration test: two emulated devices pair via code; deep link cold-start test both platforms.
...
**Accept:** sandbox purchases on both stores in TR + SA storefronts; unit tests: one purchase entitles both partners, expiry downgrades both; paywall goldens per locale.
...
PIN/biometric lock; discreet alternate icon (both platforms); KVKK/PDPL export + cascade-delete Functions (with partner notification); settings; store metadata TR/EN via Fastlane; performance pass (cold start <2s on mid-range Android reference device); crash-free instrumentation; closed-beta build.

**Accept:** delete cascade integration test (couple data gone, partner notified, entitlement handled); lock bypass attempts covered by widget tests; `release.yml` produces signed builds to TestFlight/Play internal.
