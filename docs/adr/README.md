...
| [019](019-data-rights-export-cascade-delete.md) | KVKK/PDPL data rights: **the couple is terminal** — A's hard delete removes the entire shared thread, both halves (B's couple-thread answers are mixed personal data about A; the author-split alternative recorded for the founder), B keeps everything that was ever B's alone; one idempotent **resumable** cascade callable (`deletions/{uid}` cursor, detach-B-atomically-then-sweep ordering, auth deletion LAST so a failed run stays re-drivable), kill-mid-cascade convergence test-pinned per step; partner notified via an atomic server-truth `users/{B}.coupleEnded` field (the push is deliberately name-free by design and rides the existing seams); entitlement "handled" = the mirror dies with the couple (absent doc = free) while **billing continues until cancelled in App Store — said plainly, never claimed otherwise**; export = in-app versioned JSON of strictly the requester's own data (supersedes §8's "email" parenthetical — no new operator dependency), no premium gate; the `resolveDiscreet` override finally filled (`users.notificationPrivacy` via a settings Function + rules freeze); zero client delete rules loosened | Accepted |
