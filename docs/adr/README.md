...
| [017](017-coach-chat-ui-window-persistence-personas.md) | Coach chat UI: one premium surface with a persona switcher (free tier renders NOTHING), transcript-derived windows that RETAIN crisis turns (help-sticky, explicit new-conversation exit), ephemeral in-memory threads (the private-thread decision — founder owns retention), disclaimer single-homed in ARB behind a new local-flag seam, code-first app taxonomy for the frozen wire, server-side register-aware persona scaffolds | Accepted |
