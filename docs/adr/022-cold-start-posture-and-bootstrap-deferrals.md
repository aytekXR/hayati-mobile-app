# ADR-022: Cold-start posture — instrument honestly, pin the pre-frame await set, defer App Check + tz parse past first frame, and refuse the fake <2s pass
...
| `activateAppCheck` | **no** | only needed before the first *attested backend call*, which requires user interaction; enforcement is OFF in both consoles until item-4 on-device verification (recorded M1.3/M5 posture) |
...
## Decision 1 — Two deferrals to a post-first-frame callback; one parallelization; everything else stays

`main_dev.dart`/`main_prod.dart` (edited in lockstep, as always):

- **`activateAppCheck` and `ensureCoupleTimeZonesInitialized` move to a single `addPostFrameCallback`** registered after `runHayati` — deterministic "after first frame", not an event-queue race against the first vsync. Ordering invariant, written at the call site: App Check activation must complete before any attested backend call; post-frame is safe because the earliest backend call requires a user interaction that cannot beat the first frame, and enforcement is console-OFF until item 4 verifies attestation on-device anyway. The tz parse keeps its lazy in-function guard as the correctness backstop; the post-frame call remains purely a warm-up.
- **`SharedPreferences.getInstance()` and `readInitialLockSnapshot(...)` overlap via `Future.wait`** — two independent platform-channel round-trips that both must complete before frame one; overlapping them shortens the critical path without changing what frame one knows. The lock read keeps its degraded-snapshot semantics untouched (the `Future.wait` wraps the same two futures; failure handling stays inside `readInitialLockSnapshot`).
- Net critical path becomes: binding → Firebase → Crashlytics → (prefs ∥ lock snapshot) → `runApp`. Two channel round-trips and a CPU parse leave the path; zero security or product semantics change.

## Decision 2 — The pre-frame await set is PINNED by a source-sentinel test; the trace is diagnostics, the sentinel is the gate

The durable form of the deferred-init audit is not a flaky timer but a **source sentinel** (the S020 pattern): a test that reads `main_dev.dart` + `main_prod.dart` as source and asserts (a) the exact allowed set of `await`s before `runHayati` — `initializeFirebase`, `initializeCrashlytics`, and the one `Future.wait` over `SharedPreferences.getInstance` + `readInitialLockSnapshot` — (b) lockstep equality of the two entrypoints' bootstrap shape, and (c) that `activateAppCheck`/`ensureCoupleTimeZonesInitialized` appear only in the post-frame block. Anyone adding a pre-frame await must edit the sentinel in the same diff — the addition becomes a *reviewed decision*, which is the entire point. The sentinel is mutation-checked at authoring time (move a call pre-frame, watch exactly the sentinel go red, restore).
...
- App Check activation now completes milliseconds *after* first frame; the theoretical pre-activation window is accepted because enforcement is console-OFF (recorded posture) and the earliest attested call needs user interaction. When item 4 flips enforcement ON, the on-device checklist re-verifies sign-in under enforcement.
