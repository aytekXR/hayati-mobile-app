...
- **Status:** Accepted (**rev 3** — rev 2 folded the pre-code review's 4 blocking + 6 serious + 8 minor; rev 3 folds the **post-implementation** review's 1 blocking + 1 serious + 5 minor, all 12 of whose findings survived double verification. Both reviews are recorded in `past-prompts.md` Session 025; rev 1 is in git history)
...
- **every row of D2's table** — a successful `pull_request` run → silent, no POST; the same needs on a `push` → payload; a *failing* `pull_request` → payload (failures are never suppressed); a successful **`workflow_dispatch`** → payload; a `cancelled` run → silent. (The dispatch row is not decoration: hand-firing `gh workflow run ci.yml --ref main` is how a session *recovers* a lost verdict, and the post-implementation review confirmed that without a test for it, widening the suppression guard to also swallow dispatch successes passes every other case — the exact "policy protected by nothing" this decision exists to prevent.)
- **Slack mrkdwn escaping** — `<`, `>` and `&` arrive escaped in the branch name, actor and commit subject (D5);
...
- **`jq` missing** → `::warning::` that *names jq* + `exit 0`, pinned by a PATH-shadow test (D1). **`curl` missing** → the same fail-open handling, but honestly: it is **not separately pinned**, because emptying `PATH` trips the jq guard first and `curl` is pre-installed on `ubuntu-latest`. Any curl *invocation* failure is caught by the generic POST-failed `::warning::` regardless, so the behavioural invariant holds — but the guard itself is not mutation-checked, and rev 2 claimed it was.
...
**`jq --arg` is not enough — the branch name, actor and commit subject are also escaped for Slack's renderer** (`&` → `&amp;`, `<` → `&lt;`, `>` → `&gt;`, in that order). `--arg` makes a value safe as *JSON* and safe from the *shell*; it does nothing about **mrkdwn**, which Slack renders client-side after delivery. `feat/<!channel>` is a **valid git branch name** (`git check-ref-format` accepts it, GitHub permits it), and Slack renders `<!channel>` as a live **@channel mention** — a CI notifier that anyone able to name a branch can use to ping the whole channel. Worse, `<url|label>` renders as a hyperlink whose visible label may disagree with its target: a message *from CI* that can lie about where it points. The escape is four characters of `sed` and it closes the class; a self-test pins it, and dropping the escape turns that test red. (Recorded rather than merely fixed, because the pre-code review's injection rule stopped one layer short — "safe JSON" is not "safe message".)
...
- **(a)** Reorder session-rules §3 so the session watches the main run to completion *before* pushing the close commit — correct in spirit, but it makes a durable CI guarantee depend on an agent remembering a procedure (this repo prefers the mechanism to the promise), and it forces every session to idle 25–50 min. Recorded in `session-rules.md` as *good practice*, not as the mechanism.
- **(b1)** `cancel-in-progress: false` outright — throws away the PR billing protection Session 004 was burned into.
- **(b2)** `cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}` with the group still ref-keyed. **Rev 2 collapsed this into (b1) and dismissed it for the wrong reason** (the post-implementation review caught that): it *does* preserve the PR billing protection — the expression is `true` on any PR ref — and on `main` it makes pushes **serialize** instead of cancelling, which would also save the verdict. Billing exposure is identical to D8's, since the docs-only close commit skips both macOS jobs either way (ci-debt #17). The real reason D8 wins is **latency**: (b2) keeps one group per ref, so the close commit's run would *queue behind* the still-running `integration-emulator` (25–50 min) before starting, and the session would sit watching a queued run. D8 gives each push its own group, so both run immediately and neither cancels the other.

## Decision 9 — The pre-existing hardcoded webhook: superseded, and it must be ROTATED

A local, **never-pushed** branch `chore/slack-notifications` (commit `13f1e6d`) adds a `slack-notify` job to `ci.yml` with a **live Slack webhook URL hardcoded in plaintext in the workflow file** — not a secret reference. GitHub's push protection is what stopped it reaching the remote.

**This is not a new discovery, and that is the alarming part:** `operator-expected.md` §5 has carried it as an open item **since Session 005** — "rotate the webhook in Slack (treat it as leaked)" — and roughly twenty sessions later it is still open, i.e. **still unrotated**. The pre-code review re-surfaced it; the post-implementation review then caught that §5 also instructs the founder to *"rework/land the branch"*, which this ADR now directly contradicts. A stale operator item that tells the founder to push a credential is worse than no item at all. Hence:

- **The branch is superseded by this ADR and must not be merged or pushed.** Its design is the ams-pulse copy-paste (both defects of D1, plus the URL in the clear). `operator-expected.md` §5's "then rework/land the branch" is **deleted, not amended** — this session ships what that branch was for, correctly.
...
- **The operator item must name the secret's SCOPE explicitly.** The only secret precedent in this repo is ADR-021's three `ASC_*` **environment** secrets, so a founder pattern-matching from it would put `SLACK_WEBHOOK_URL` in the `release` environment — where the notifier (which deliberately has no `environment:` binding, D4) can never see it. The failure is silent by construction: the run emits the same `::notice::` as a repo with no webhook at all. The item therefore carries the verbatim command **and** the "NOT `--env release`" warning **and** a confirmation step.
