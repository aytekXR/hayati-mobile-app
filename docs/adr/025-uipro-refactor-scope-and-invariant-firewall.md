...
- **Status:** Accepted (rev 2) — **the eight-slice arc is COMPLETE (Session 035, 2026-07-23).** All 48 inventoried surfaces have been through a slice or are recorded parity-only (Class F) / unreachable (Class S); slices 0–8 shipped (see the dated slice notes below). Two design questions the arc surfaced remain open for the founder: **#67** (the muted/outline tokens — the one gate on a final settings-polish pass) and **#63** (Phosphor — a whole-app decision that would be its own slice 1.5). _(rev 2 folded the pre-code adversarial review's 1 blocking + 7 serious + 7 minor, all 15 of which survived double verification by a refuting skeptic and a governing-docs adjudicator out of 26 raised; plus one defect the review missed and the author's own verification caught, the Phosphor divergence. Rev 1 is in git history; the review is recorded in `past-prompts.md` Session 026.)_
...
- **Slice 7 (legal & consent) shipped in Session 034** — `ConsentGateScreen`, `LegalScreen` (hub), `LegalDocumentScreen` + `legal_renderer.dart` (Class **G**, layout-only). Two auditors + a governing-docs adjudicator found the consent gate and the legal hub already token-clean and layout-correct (all four ADR-023 consent escapes present, the digest untouched, `SettingsErrorLine` shared not forked, RTL correct), and one genuine defect in the renderer — **ONE change, 3 goldens, 15 byte-identical**:
  **(1) The legal document's own h1 title was rendering LIGHTER than its `##` section headings.** `legal_renderer.dart` styled the `#` document title with `textTheme.headlineSmall` — a slot `TypographyTokens` never defines, so it fell through to Material 3's default (24sp/**w400**), while `##` section headings use `titleMedium` (16sp/**w600**). The document's own title (the parser's declared h1) was thus the ONE place in the app using an undefined type role, and it read *lighter* than its sub-headings — a weight-hierarchy inversion. Fixed to `headlineMedium`, the DEFINED h1 role (24sp/w700 per `TypographyTokens`' own annotation and brandkit §3 "H1 = 24/700"). LOOKED at the regenerated goldens: the title now leads in both size and weight, matching the bold app-bar title, with the sections correctly subordinate. 3 goldens: `legal_document_screen` `privacy_policy` × {en.ltr, ar.rtl, tr.ltr} (the naturals — a long document is captured natural-direction only); the 9 `consent_gate` + 6 `legal_screen` hub goldens byte-identical.
  **(2) The Class-G guarantees hold — no copy, no escapes, no bytes touched.** The change is a type-role substitution in the renderer: no `consent*`/`legal*` string reworded (frozen digest green), the four consent escapes untouched (the change is in the renderer, not the gate), the legal markdown BYTES untouched (the `legal_assets_drift_test` byte-faithful test green), `SettingsErrorLine` not forked, no `onSurfaceVariant`/`outline` (#67 not implicated — the consent gate's existing `onSurfaceVariant` age-statement caption was left exactly as-is, being #67 itself). No motion (navigated surfaces). 75 legal + guard tests green.
- **Slice 8 (the lock — parity only) shipped in Session 035 as a ZERO-CHANGE parity proof, and THE ARC IS NOW COMPLETE.** Class **F** — the four lock surfaces (`PrivacyGuard`, `PrivacyShieldCover`, `LockScreen`, `PinKeypad`). Class F forbids restructuring, so slice 8's deliverable is not a change but a PROOF that the lock is byte-identical and its firewall intact, and the closing of the arc. Verified by direct test RE-RUN (130 lock + guard tests green):
  **(1) All 18 lock goldens + the 2 `probe` PNGs BYTE-IDENTICAL** (`lock_screen_golden_test` + `rtl_mirror_net_test` pass with no `--update-goldens`) — the lock reached byte-identity at slice 1 (D6's carve-out), and it never drifted across slices 2–7. **(2) The four ADR-018 lock invariants hold:** the D5.i forbidden-API sentinel green (no `showDialog`/`Overlay`/`Tooltip`/`tooltip:`/`IconButton`/text-selection/`ScaffoldMessenger.of` under `LockScreen`); the no-invalidate sentinel green (nothing calls `ref.invalidate(privacyLockControllerProvider)`); the `biometricOnly: true` source-contract test green; and by source, `PrivacyShieldCover` keeps its direct `ColorTokens.night` (FLUTTER-4), the `PinKeypad` keeps its explicit `TextDirection.ltr`, and the `Offstage`+`TickerMode` gating shape is unchanged. **(3) Zero changes — the arc's most conservative outcome on its most safety-critical surface, exactly as D-Consequences predicted ("the lock surfaces get almost no refactor … ADR-018's four invariants are worth more than visual consistency").**
  **THE ARC IS COMPLETE (D7's own criterion).** All 48 inventoried surfaces have now been through a slice or are recorded parity-only/unreachable: slice 0 built the firewall; slice 1 filled the Material floor; slices 2 (reveal) + 3 (pairing activation) added the two §6 signature motions + composition; slice 4 (commerce = one h1→h2) + slice 7 (legal = one undefined-slot→h1) were single defensible fixes; slices 5 (coach) + 6 (settings, #67-blocked) were documented already-compliant reviews; slice 8 (the lock) is the parity proof. **The redesign the founder directed (2026-07-14) is done. The remaining MVP work is entirely operator-blocked: M5.3 (live coach) on item 6, the first deploy on item 2 (Blaze), the on-device/TestFlight lane on item 4 (Apple enrollment). #67 (muted/outline tokens) remains the one open design question whose answer would unblock a short settings-polish follow-up; #63 (Phosphor) a whole-app decision that would be its own slice 1.5.**
- **Post-arc follow-up — #74 (the §6-unfold DRY) resolved in Session 036 (2026-07-24).** Not a slice — a tracked, pixel-neutral tidy-up the arc deliberately deferred (slice 3 recorded the ~15 duplicated lines as #74 rather than smuggle a DRY into a golden-frozen slice). Slice 2's file-local `_RevealUnfold` and slice 3's shared `SoftUnfoldReveal` were byte-behaviourally identical except their `@visibleForTesting` `Opacity` key string. Design **A1** (chosen over full-DRY): `SoftUnfoldReveal` gained an optional `opacityKey` param (default `softUnfoldOpacityKey`); `paired_home_screen` now renders `SoftUnfoldReveal(opacityKey: revealUnfoldOpacityKey, …)` and the `_RevealUnfold` class + its now-unused `motion_tokens` import are deleted. **A1 was chosen precisely because #74 says "keep `revealUnfoldOpacityKey`":** it preserves that surface's test seam and leaves the daily-reveal motion+haptic tests VERBATIM (zero test churn — no risk of silently weakening a §6/haptic assertion), and it keeps the two reveal surfaces independently addressable in tests. Full-DRY B (delete the key, point the daily-reveal tests at `softUnfoldOpacityKey`) would have churned those assertions to satisfy a DRY the issue did not ask for. **D8 proof:** `git status --porcelain` shows only the two `.dart` lib files — **zero golden moved** (both `paired_home_screen_golden_test` and `partner_preview_screen_golden_test` pass with no `--update-goldens`); `flutter analyze` clean; 1,449 app tests green including the three firewall guards (lock sentinel, token parity, frozen digest); coverage 86.44%. The haptic host (`_PairedQuestionViewState.didUpdateWidget`, at-most-once) and the daily reveal's x4 answer grouping were untouched. Reviewed by a 4-lens adversarial diff pass (correctness/pixel-neutrality · test-integrity · DRY-design · governing-docs/firewall) with per-finding refuting-skeptic + governing-docs-adjudicator verification — **verdict CLEAN** (2 findings raised, both refuted: an `opacityKey`-is-a-test-seam-on-production-API observation, and a coverage nit that the skeptic itself disproved by pointing at the daily-reveal tests). The coverage nit's fair *maintainability* point — the shared widget's OWN suite shouldn't lean on a feature screen to cover its new param — was nonetheless closed by adding a direct override-path test to `soft_unfold_reveal_test.dart` (asserts a caller-supplied `opacityKey` is stamped and the default is then unused), so the param routing is self-proven.
