...
- Tag runs re-run the integration suite on macOS runners (~binds a release to ~30-40 min of runner time). Accepted: a release without the strongest available signal would be cheaper and worse. **Rev note — 2026-07-26 (S041, ADR-029 D6):** this bullet originally said *"10×-billed macOS minutes"*. That premise is void — the repo is **PUBLIC** and GitHub-hosted standard runners, macOS included, are **free for public repositories**. The trade-off still stands on *wall-clock* (a release is bound to that runner time either way); only the billing claim was wrong. Whether the cost-motivated gates elsewhere should be relaxed is issue **#100**. **D5's "likeliest Mac-era fix" prediction has since come true and been resolved by ADR-029 D1** — `DEVELOPMENT_TEAM` is now committed, so the archive no longer fails for that reason.
