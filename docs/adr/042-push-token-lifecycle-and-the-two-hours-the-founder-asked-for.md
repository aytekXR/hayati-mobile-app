...
- **Status:** Accepted — **D1–D5 IMPLEMENTED AND SHIPPED.** S062: D1 (#187). S063: D3/D4 (#190), D2 in full (#191 the plugin, #194 the entitlement) after the founder authorised enabling the App ID capability from CI (#193). `aps-environment` is signed into **build 115**. **D6 (the permission prompt) remains deferred** — it is a UI surface, and iOS gives one shot per install. **No push has yet been delivered**: Firebase still has no APNs `.p8`, which is console-only. (rev 2 — folds the pre-code adversarial design review. 36 findings raised across 5 lenses; **2 confirmed** and fixed in place, 7 re-adjudicated 3-lens after the first round's verifiers ran against a worktree that did not contain this file. See the review record at the end.)
