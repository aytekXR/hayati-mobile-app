...
- **~~`Gemfile.lock` is still absent.~~ DISCHARGED — issue #120, Session 048.** ADR-021 D6 tied the debt to
  *"the first time fastlane is exercised"*, which had happened three times over without anyone noticing the
  condition had been met. The original blocker never lifted — there is no Ruby on the Linux dev box — so the
  lock is generated by CI instead of by hand: `.github/workflows/gemfile-lock.yml` resolves it on the **same
  runner and Ruby** the release lane installs on (a lock resolved on ubuntu can omit the darwin platform, and
  `bundle install` then refuses it on macOS), proves it with **`bundle install --frozen`** — the `npm ci` of
  this ecosystem, applying S044's lesson directly — and hands it back as an artifact for a human to commit.
  It pins **fastlane 2.237.0** and 96 gems, and `tool/release_lane_lint.dart` rule 5 now fails the cheap
  ubuntu preflight if the lock and the Gemfile ever disagree.
