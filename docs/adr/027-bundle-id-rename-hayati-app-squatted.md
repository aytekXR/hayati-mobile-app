...
> **Rev note — 2026-07-26 (Session 041, ADR-029 D5): Phase 2 is DONE, and it landed inside this ADR's own commit.** The paragraph above describes the *plan*; the outcome was faster than the plan. The founder registered the new iOS app in **both** projects during Session 037, so the regen went into the **same** merge (`ce80908`, PR #90) — `firebase_options_{dev,prod}.dart`, `google_sign_in_config.dart`, the `Info.plist` `CFBundleURLSchemes` and `firebase_bootstrap_test.dart` all carry the new values on `main`, and the "transient split" never existed in a commit at all. **Nothing about Phase 2 is outstanding.** Session 041 re-verified it two independent ways: `firebase apps:sdkconfig IOS <appId>` for both projects (live `Hayati iOS (beyondkaira)` apps, dev `1:870954957461:ios:98d074e9af5ced5c17f99c` and prod `1:419979715508:ios:c8c0e5c1fdfadf9d64c8e1`, both `BUNDLE_ID = com.beyondkaira.hayati`) matches every committed value byte-for-byte, and `git show ce80908` shows the change. The reasoning above is kept because it is *why* the change was safe; only the pending framing was stale. **Decision 3's last sentence is likewise resolved by ADR-029 D1: `DEVELOPMENT_TEAM = UH7MXG7Z94` is now COMMITTED** (an identifier, not a credential — four grounds recorded there), so it is no longer a local-only working-tree edit.
