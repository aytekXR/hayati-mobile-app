...
- **Secret binding:** `coachProxy` declares `secrets: ['LLM_API_KEY']` (the `RC_WEBHOOK_TOKEN` precedent). The deploy **fails closed and loud** until the secret exists — never a silent green.
  ...
  > **Rev note (S042) — the precedent covers only half of this path.** The `RC_WEBHOOK_TOKEN` attempt (S040) proved the **missing-secret** direction: the deploy stopped with *"Secret … not found or has no versions"*. It never reached the **present-secret** direction, because that secret still does not exist. `LLM_API_KEY` **does** exist, so `coachProxy`'s deploy takes a path this repo has never exercised: firebase-tools must grant `roles/secretmanager.secretAccessor` on the secret to the runtime service account before the revision can bind the env var. That auto-grant was documented firebase-tools behaviour and **not** an in-repo finding when this note was written. **It is now — verified at the S042 deploy, and recorded here rather than left understated:** `coachProxy` redeployed to `hayatiapp-dev` at `2026-07-26T04:24:12Z`, revision **`coachproxy-00004-van`**, `state: ACTIVE`, carrying `secretEnvironmentVariables: [{key: LLM_API_KEY, secret: LLM_API_KEY, version: '1'}]`, and the secret's IAM policy now holds **`roles/secretmanager.secretAccessor` → `serviceAccount:870954957461-compute@developer.gserviceaccount.com`** (the runtime SA) — a binding nobody added by hand. Cold start was clean and an unauthenticated call returned **HTTP 401** *"coachProxy requires a signed-in caller."* rather than a 5xx. The safety point in the original note still stands and is why this was checked at all: had the grant failed, the adapter would fail closed to `unconfigured` — a coach that honestly says "unavailable" — which is **silent from the app's side**, so it must be confirmed by reading the revision and the IAM policy, never inferred from the app's behaviour.
